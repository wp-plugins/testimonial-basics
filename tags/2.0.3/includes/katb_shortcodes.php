	...
	if( $katb_error != '') {
		...
		for ( $i = 0 ; $i < $katb_tnumber; $i++ ) {
			if ( $katb_options['katb_use_formatted_display'] == 1 ) {
				...
				$katb_html .= '<span class="katb_test_text">'.stripcslashes($katb_tdata[$i]['tb_testimonial']).'</span>';
				$katb_html .= '<span class="katb_test_meta"><br/>'.stripcslashes($katb_tdata[$i]['tb_name']);
			} else {
				$katb_html .= '<div class="katb_test_box_basic">';
				$katb_html .= '<span class="katb_test_text_basic">'.stripcslashes($katb_tdata[$i]['tb_testimonial']).'</span>';
				$katb_html .= '<span class="katb_test_meta_basic"><br/><b>'.stripcslashes($katb_tdata[$i]['tb_name']).'</b>';
			}
			...
			if ($katb_options['katb_show_website'] == 1) {
				if ( $katb_tdata[$i]['tb_url'] != "" ) $katb_html .= ', <i><a href="'.esc_url($katb_tdata[$i]['tb_url']).'" title="Testimonial_author_site" >Website</a></i>';
			}
			$katb_html .= '</span>';
			$katb_html .= '</div>';
		}
